...
export default function RestaurantDetail() {
  ...
  return (
    <div className="max-w-3xl mx-auto pb-8">
      ...
      <div className="relative">
        ...
        <div className="absolute bottom-0 inset-x-0 p-6 text-primary-foreground">
          <div className="flex items-center gap-2 mb-2">
            ...
            {restaurant.isOpen ? (
              <Badge className="bg-accent/90 text-accent-foreground border-0 text-xs px-3 py-1 shadow-md">Open Now</Badge>
            ) : (
              <Badge className="bg-destructive/90 text-destructive-foreground border-0 text-xs px-3 py-1 shadow-md">Closed</Badge>
            )}
          </div>
          <h1 className="text-3xl md:text-4xl font-display font-bold drop-shadow-lg">{restaurant.name}</h1>
        </div>
      </div>
        ...
        {items.map((item) => (
          <Card
            ...
            className={`overflow-hidden border-0 shadow-md transition-all duration-300 ${
              !item.isAvailable ? "opacity-60" : "hover:shadow-xl hover:-translate-y-0.5"
            }`}
          >
      ...
      {/* Floating cart CTA */}
      <div className="fixed bottom-20 md:bottom-6 inset-x-0 px-5 max-w-3xl mx-auto z-30">
        <Button
