...
process.env.MONGO_URI             = 'mongodb://localhost:27017/mealstack_payment_test';
...
describe('Payment Service – smoke tests', () => {
  describe('GET /health', () => {
    it('returns 200 with status ok and service name', async () => {
      const res = await request(app).get('/health');
      expect(res.status).toBe(200);
      expect(res.body).toMatchObject({ status: 'ok', service: 'payment' });
    });
  });
